...
class EditMatchProduct extends StatefulWidget {
  ...
  const EditMatchProduct(
      {Key? key, required this.product1, required this.product2})
      : super(key: key);
...
class _EditMatchProductState extends State<EditMatchProduct> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance!.addPostFrameCallback((_) async {
      ...
      List<String> preselectedColors =
          List<String>.from(product1['p_mixmatch_colors'] ?? []);
      List<int> preselectedIndices = preselectedColors
          .map((color) => controller.allColors
              .indexWhere((element) => element['name'] == color))
          .where((index) => index != -1)
          .toList();
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.product1['p_mixmatch_collection'] is List) {
      List<String> collections = List<String>.from(widget
          .product1['p_mixmatch_collection']
    ...
    return FutureBuilder<Rxn<List<Product>>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          int initialIndex = products.indexWhere(
              (product) => product.mixmatch == widget.product1['p_mixmatch']);
    ...
    return FutureBuilder<Rxn<List<Product>>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          List<Product> products = snapshot.data!.value!;
          int initialIndex = products.indexWhere(
              (product) => product.mixmatch == widget.product2['p_mixmatch']);
